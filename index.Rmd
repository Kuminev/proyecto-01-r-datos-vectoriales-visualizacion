...
```{r}
asia_paises <- world[, c("continent", "lifeExp", "name_long"), drop=TRUE]

asia_continente <- subset(
  world[, c("continent", "lifeExp", "name_long"), drop=TRUE],
  continent == "Asia" & lifeExp>=80
)

names (asia_continente)=c("Continente", "Expectativa", "País")

ggplot(asia_continente, aes(x=reorder(País, -Expectativa), y=Expectativa)) +
  ggtitle("Expectativa de vida en paises asiáticos") +
  xlab("País") + 
  ylab("Años de expectativa vida") +  
  geom_col() +
  geom_text(aes(label=Expectativa), vjust=-0.3, size=3.5)+
  theme_bw()
...
```{r}
Oceania_conti <- world[world$continent=="Oceania",]

plot(Oceania_conti["lifeExp"], main="Expectativa de vida")
...
```{r}
centro_america1= world[world$name_long == "Belize" |
                       world$name_long == "Guatemala" |
                       world$name_long == "Honduras" |
                       world$name_long == "El Salvador" |
                       world$name_long == "Nicaragua" |
...
centro_america1 <- subset(
  centro_america1[, c("subregion", "lifeExp", "name_long")],
   lifeExp>=74
  )

plot(centro_america1["lifeExp"], main="Expectativa de vida")
...
```{r}
sur_america<- world[world$continent=="South America",]

sur_america2 <- subset(
  sur_america[, c("continent", "lifeExp", "name_long", "area_km2", "pop")], area_km2>2500000)


sur_america3 <- subset(
  sur_america[, c("continent", "lifeExp", "name_long", "area_km2", "pop")], pop>=40000000)                 
                    
```
```{r}
plot(sur_america2["area_km2"], main="Área mayor a 2.5 mill")
```
```{r}
plot(sur_america3["pop"], main="Población mayor a 40 mill")
```